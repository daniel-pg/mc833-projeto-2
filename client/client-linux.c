...
int main(int argc, char *argv[]) {
    ...
    for(p = srv_info_tcp; p != NULL; p = p->ai_next) {
        if (-1 != (sockfd_tcp = socket(p->ai_family, p->ai_socktype, p->ai_protocol))) {
            break;
        }
    }
    ...
    // Create UDP socket
    for(p = srv_info_udp; p != NULL; p = p->ai_next) {
        if (-1 != (sockfd_udp = socket(p->ai_family, p->ai_socktype, p->ai_protocol))) {
            break;
        }
    }
    ...
    if (-1 == connect(sockfd_tcp, p->ai_addr, p->ai_addrlen)) {
